...
func TestAddItem(t *testing.T) {
	...
	setup := func(t *testing.T, id, itemID string) Test {
		...
		req := httptest.NewRequest(http.MethodPut, "/issues/"+id+"/items/"+itemID, nil)
	...
	t.Run("Links item and returns refreshed issue", func(t *testing.T) {
		t.Parallel()

		deps := setup(t, "42", "7")
		deps.Items.EXPECT().LinkToIssue(gomock.Any(), int64(42), int64(7)).Return(nil)
		deps.Issues.EXPECT().Find(gomock.Any(), int64(42)).Return(digest.Issue{ID: 42, Status: digest.IssueStatusDraft}, nil)

		err := deps.Handler.AddItem(deps.Context)
	...
	t.Run("Invalid id returns 400", func(t *testing.T) {
		...
		deps := setup(t, "abc", "7")
		_ = deps.Handler.AddItem(deps.Context)
	...
	t.Run("Not found returns 404", func(t *testing.T) {
		...
		deps := setup(t, "42", "7")
		deps.Items.EXPECT().LinkToIssue(gomock.Any(), int64(42), int64(7)).Return(store.ErrNotFound)
	...
	t.Run("Non-draft issue returns 409", func(t *testing.T) {
		...
		deps := setup(t, "42", "7")
		deps.Items.EXPECT().LinkToIssue(gomock.Any(), int64(42), int64(7)).Return(digest.ErrIssueNotDraft)
	...
	t.Run("Repo error returns 500", func(t *testing.T) {
		...
		deps := setup(t, "42", "7")
		deps.Items.EXPECT().LinkToIssue(gomock.Any(), int64(42), int64(7)).Return(errors.New("db error"))
