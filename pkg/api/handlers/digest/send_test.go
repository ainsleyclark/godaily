...
func TestSend(t *testing.T) {
	...
	t.Run("Sends digest successfully on a weekday", func(t *testing.T) {
		t.Parallel()
		...
		synctest.Test(t, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodGet, "/digest/send", nil)
			deps := setup(t, req)
			deps.Runner.EXPECT().SendDigest(gomock.Any(), gomock.Any(), false).Return(nil)

			err := deps.Handler.Send(deps.Context)
	...
	t.Run("Send digest error returns internal server error", func(t *testing.T) {
		t.Parallel()
		...
		synctest.Test(t, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodGet, "/digest/send", nil)
			deps := setup(t, req)
			deps.Runner.EXPECT().SendDigest(gomock.Any(), gomock.Any(), false).Return(errors.New("send failed"))

			_ = deps.Handler.Send(deps.Context)

			assert.Equal(t, http.StatusInternalServerError, deps.Recorder.Code)
		})
	})

	t.Run("Skips send on weekend", func(t *testing.T) {
		t.Parallel()
